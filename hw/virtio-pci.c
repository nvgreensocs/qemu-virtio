...
static int virtio_net_pci_init(VirtIOPCIProxy *vpci_dev)
{
    ...
    vdev = qdev_create(BUS(vpci_dev->bus), "virtio-net");
...
static int virtio_balloon_pci_init(VirtIOPCIProxy *vpci_dev)
{
    ...
    qdev = qdev_create(BUS(vpci_dev->bus), "virtio-balloon");
...
static int virtio_rng_pci_init(VirtIOPCIProxy *vpci_dev)
{
    ...
    qdev = qdev_create(BUS(vpci_dev->bus), "virtio-rng");
    ...
    virtio_rng_set_conf(qdev, &(vrng->conf));
...
{
    VirtIORngPCI *vrng = VIRTIO_RNG_PCI(obj);
...
static int virtio_serial_pci_init(VirtIOPCIProxy *vpci_dev)
{
    ...
    qdev = qdev_create(BUS(vpci_dev->bus), "virtio-serial");
    ...
    if (vpci_dev->nvectors == DEV_NVECTORS_UNSPECIFIED) {
        set_virtio_device_nvectors(vpci_dev->bus,
                                   dev->serial.max_virtserial_ports + 1);
    } else {
        set_virtio_device_nvectors(vpci_dev->bus, vpci_dev->nvectors);
    }
    return 0;
