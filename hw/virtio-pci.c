        ...
        VirtIOTransportLink *trl)
{
    ...
    case VIRTIO_ID_BALLOON:
        vdev->nvectors = s->nvectors;
        pci_config_set_device_id(s->pci_dev.config,
                                 PCI_DEVICE_ID_VIRTIO_BALLOON);
        pci_config_set_class(s->pci_dev.config, PCI_CLASS_OTHERS);
        virtio_init_pci_(s, vdev);
        s->nvectors = vdev->nvectors;
        break;
    case VIRTIO_ID_SCSI:
        vdev->nvectors = s->nvectors;
        pci_config_set_device_id(s->pci_dev.config, PCI_DEVICE_ID_VIRTIO_SCSI);
        pci_config_set_class(s->pci_dev.config, PCI_CLASS_STORAGE_SCSI);
        virtio_init_pci_(s, vdev);
        s->nvectors = vdev->nvectors;
        break;
    case VIRTIO_ID_CONSOLE:
    {
        ...
        pci_config_set_class(s->pci_dev.config, PCI_CLASS_COMMUNICATION_OTHER);
        virtio_init_pci_(s, vdev);
        s->nvectors = vdev->nvectors;
        break;
    }
    case VIRTIO_ID_9P:
        vdev->nvectors = s->nvectors;
        pci_config_set_device_id(s->pci_dev.config, 0x1009);
        pci_config_set_class(s->pci_dev.config, 0x0002);
        virtio_init_pci_(s, vdev);
        s->nvectors = vdev->nvectors;
        break;
    default:
