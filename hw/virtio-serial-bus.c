...
/***************** VirtIOSERIAL-PCI Device *******************/
/* This device create a virtio-pci and virtio-serial device  */


static int virtio_serial_pcidev_init(DeviceState *dev)
{
    VirtIODevice *vdev;
    VirtIOSerState *s = VIRTIO_SERIAL_FROM_QDEV(dev);
    PCIBus *rootpcibus;
    ...
    /* Link Virtio SERIAL to Virtio PCI */
...
{
    DeviceClass *dc = DEVICE_CLASS(klass);
    dc->init = virtio_serial_pcidev_init;
