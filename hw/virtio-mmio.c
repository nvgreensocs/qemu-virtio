...
static void virtio_mmio_update_irq(DeviceState *d, uint16_t vector)
{
...
static unsigned int virtio_mmio_get_features(DeviceState *d)
{
    VirtIOMMIOProxy *proxy = VIRTIO_MMIO(d);
    return proxy->host_features;
}

static int virtio_mmio_load_config(DeviceState *d, QEMUFile *f)
{
    VirtIOMMIOProxy *proxy = VIRTIO_MMIO(d);
...
static void virtio_mmio_save_config(DeviceState *d, QEMUFile *f)
{
    VirtIOMMIOProxy *proxy = VIRTIO_MMIO(d);
...
static void virtio_mmio_device_plugged(DeviceState *d)
{
    VirtIOMMIOProxy *proxy = VIRTIO_MMIO(d);
    proxy->host_features |= (0x1 << VIRTIO_F_NOTIFY_ON_EMPTY);
    proxy->host_features = virtio_device_get_features(proxy->bus,
                                                      proxy->host_features);
}
