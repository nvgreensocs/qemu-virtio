 ...
 */
VirtIOTransportLink *virtio_find_transport_by_id(const char *name)
{
...
/*
 * Find transport device by its DeviceState.
 * Used by virtio-*-pci devices.
 */
VirtIOTransportLink *virtio_find_transport_by_device_state(DeviceState *dev)
{
    VirtIOTransportLink *trl;
...
{
    if (trl->used) {
        /* device init already done : transport already used ? */
        error_report("%s transport in use.", trl->tr->id);
        return -1;
    }
    if (trl->cb) {
        trl->used = true;
